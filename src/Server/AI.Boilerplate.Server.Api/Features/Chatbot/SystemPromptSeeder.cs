...
public class SystemPromptSeeder : IDataSeeder
{
    public async Task SeedAsync(AppDbContext dbContext, CancellationToken cancellationToken)
    {
        var id = Guid.Parse("a8c94d94-0004-4dd0-921c-255e0a581424");
        
        if (await dbContext.SystemPrompts.AnyAsync(sp => sp.Id == id, cancellationToken) is false)
        {
            await dbContext.SystemPrompts.AddAsync(new SystemPrompt
            {
                Id = id,
...
- ### 未解决的问题：
    ...
    - If your email ({{UserEmail}} variable) is empty, request your email.
